...
public class ChatMessageList: Control {
	...
	public void AddChatMessage(ChatMessageSide side, string name, string message, Dictionary<ChatMessageOptions, int> opts) {
		GameManager.chatWindow.GetNode<AudioStreamPlayer>("NewSound").Play();
		...
		if (GameManager.chatWindow.activeChat != this.chatIndex) { GameManager.chatWindow.GetChatListItem(this.chatIndex).AddUnread(); }
		UpdatePositions();
	}
